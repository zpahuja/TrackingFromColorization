...
def main(args):
    ...
    for frame, image, gray, color, name in ds.get_data():
        ...
        stacked = np.concatenate([image, predicted, predicted_eval2], axis=1)

        for _, viz, mask, _predicted, mfi_mask, mfi_predicted in [('image', stacked, mask, predicted_col, davis_2016_mfi_mask, predicted_eval)]:
            ...
            im = Image.fromarray(mfi_mask)
            im.putpalette(color_palette.ravel())
            im.save('%s/%05d.png' % (mfi_mask_dir, frame), format='PNG')

            im = Image.fromarray(mask)
            im.putpalette(color_palette.ravel())
            im.save('%s/%05d.png' % (mask_dir, frame), format='PNG')

            h, w = mfi_predicted.shape[:2]
            segments = np.where(mfi_mask != 0)
            masked_pred = mfi_predicted[segments]
            kmeans = KMeans(
