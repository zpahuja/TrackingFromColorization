...
def main(args):
    ...
    for frame, image, gray, color, name in ds.get_data():
        ...
        predicted_eval = cv2.resize(predicted_eval, (width, height), interpolation=cv2.INTER_AREA)
        davis_2016_mfi_mask = cv2.resize(mask.astype('uint8'), (width, height), interpolation=cv2.INTER_AREA)
        ...
        if args.name == 'davis':
            ...
            predicted_eval2 = cv2.add(cv2.bitwise_and(image, image, mask=mfi_mask_inv), predicted_eval)
            predicted_eval2 = cv2.addWeighted(image, 0.3, predicted_eval2, 0.7, 0)

            # grabcut
            grabcut_mask_inv = cv2.bitwise_not(grabcut_mask)
            grabcut_eval = cv2.bitwise_and(predicted_col, predicted_col, mask=grabcut_mask)

            grabcut_predicted_image = cv2.add(cv2.bitwise_and(image, image, mask=grabcut_mask_inv), predicted_col)
            grabcut_predicted_image = cv2.addWeighted(image, 0.3, grabcut_predicted_image, 0.7, 0)

        stacked = np.concatenate([image, predicted, predicted_eval2, grabcut_predicted_image], axis=1)
        ...
        for _, viz, mask, _predicted, mfi_mask, mfi_predicted, grabcut_mask, grabcut_predicted in [('image', stacked, mask, predicted_col, davis_2016_mfi_mask, predicted_eval, grabcut_mask, grabcut_eval)]:
            ...
            grabcut_kmean_dir = os.path.join(output_dir, 'grabcut_predictions', name)
