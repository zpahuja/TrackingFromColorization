    ...
    def resize(small_axis=256, copy=False):
        def _resize(images):
            ...
            for idx, image in enumerate(images):
                ...
                width = int(small_axis if aspect_ratio <= 1 else small_axis * aspect_ratio)
                height = int(small_axis if aspect_ratio >= 1 else small_axis / aspect_ratio)
    ...
    def crop(shape, copy=False):
        def _crop(images):
            ...
            for idx, image in enumerate(images):
                ...
                image = image.reshape(
                    (height, width, -1))[start_y:end_y, start_x:end_x, :]
                images[idx] = image
