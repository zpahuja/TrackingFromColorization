...
from colorizer.feeder.dataset import Kinetics
...
def test_kinetics_get():
    kinetics = Kinetics('/data/public/rw/datasets/videos/kinetics')
    assert kinetics.get_filepath(kinetics.names[0]) == (True, '/data/public/rw/datasets/videos/kinetics/processed/---0dWlqevI.mp4')
    assert kinetics.get_filepath(kinetics.names[-1]) == (False, '/data/public/rw/datasets/videos/kinetics/processed/zzz_3yWpTXo.mp4')
    with pytest.raises(KeyError) as exception_info:
        kinetics.get_filepath('NOT_EXISTS_NAME')
